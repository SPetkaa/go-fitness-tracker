...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	vKmH := meanSpeed(action, duration)
	return ((runningCaloriesMeanSpeedMultiplier * vKmH * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH)
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	return (walkingCaloriesWeightMultiplier*weight + (math.Pow(speed, 2)/h)*walkingSpeedHeightMultiplier*weight) * duration * minInH
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	// ваш код здесь
	vKmH := swimmingMeanSpeed(lengthPool, countPool, duration)
	return (vKmH + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
}
